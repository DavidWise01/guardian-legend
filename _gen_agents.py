#!/usr/bin/env python3
"""Materialize TGL (The Guardian Legend) persona ACI badges from the .agent files
on disk. Each → full ACI complement + agents/_personas.json (emergence)."""
import os, sys, json, re
sys.stdout.reconfigure(encoding="utf-8")
HERE = os.path.dirname(os.path.abspath(__file__))
sys.path.insert(0, HERE)
import build  # guardian-legend/build.py — write_aci, NATURES

AGENTS = os.path.join(HERE, "agents")

ORDER = ["miria-the-guardian","the-fighter","the-corridors","the-defenders-of-naju",
         "the-arsenal","earth","the-guardians-charge"]
...
for fn in os.listdir(AGENTS):
    ...
    rec = {
        "name": fr.get("aci", slug), "axiom": "TGL", "emergence": em,
        "seal": fr.get("seal", ""), "origin": "TGL · The Guardian Legend",
        ...
        "conductor": "ROOT0 (catalogued into UD0)", "inputs": fr.get("series", "The Guardian Legend"),
        "source": "The Guardian Legend emergent, catalogued by ROOT0",
    }
...
print(f"wrote {len(ordered)} TGL persona ACI badges + _personas.json")
